...
int train_1layer_net(double sample[INPUTS],int label,double (*sigmoid)(double input), double weights_io[INPUTS][OUTPUTS])
{
  ...
  ***********************************************************************************************************/
	...
	classification = classify_1layer(sample, label, sigmoid, weights_io);
  ...
  */ 
   ...
   ******************************************************************************************************/
	...
	for (i = 0; i < OUTPUTS; i++) {
		...
		for (j = 0; j < INPUTS; j++) {
			activation += (sample[j])*(weights_io[j][i]);
		}
		
		activation = activation*SIGMOID_SCALE;
		activations[i] = sigmoid(activation);
 	}
}
...
void backprop_1layer(double sample[INPUTS], double activations[OUTPUTS], double (*sigmoid)(double input), int label, double weights_io[INPUTS][OUTPUTS])
{
   ...
   */
    ...
    * ************************************************************************************************/
	...
	int i, j;
	double target_correct;
	double target_incorrect;
	double (*partial_derivative)(double input);
	double first_term, second_term, third_term;
	double target;
	double activation;
	double ais[OUTPUTS];
	
	// If we're using the logistic function
	if (sigmoid == &logistic) {
		target_correct = 1.0;
		target_incorrect = 0.0;
		partial_derivative = &logistic_partial_derivative;
	// Otherwise we're using the hyperbolic function
	} else {
		target_correct = 1.0;
		target_incorrect = -1.0;
		partial_derivative = &hyperbolic_partial_derivative;
	}
	...
	for (i = 0; i < OUTPUTS; i++) {
		...
		ais[i] = activation*SIGMOID_SCALE;
	}
	...
	for (i = 0; i < OUTPUTS; i++) {	
		...
		for (j = 0; j < INPUTS; j++) {
			...
			// Second term - Partial derivative of activation fcn w.r.t. input
			second_term = partial_derivative(ais[i]);
			
			// Third term - (T_b - O_b)
			third_term = target - activations[i];
			
			// Weight update
			weights_io[j][i] += ALPHA*(first_term*second_term*third_term);
		}
	}
}
...
int train_2layer_net(double sample[INPUTS],int label,double (*sigmoid)(double input), int units, double weights_ih[INPUTS][MAX_HIDDEN], double weights_ho[MAX_HIDDEN][OUTPUTS])
{
  ...
  ***********************************************************************************************************/	
	...
	classification = classify_2layer(sample, label, sigmoid, units, weights_ih, weights_ho);
  ...
  */ 
   ...
   **************************************************************************************************/
	...
	for (i = 0; i < units; i++) {
		...
		for (j = 0; j < INPUTS; j++) {
			activation += (sample[j])*(weights_ih[j][i]);
		}
		
		activation = activation*SIGMOID_SCALE;
		h_activations[i] = sigmoid(activation);
	}
	...
	for (i = 0; i < OUTPUTS; i++) {
		...
		for (j = 0; j < units; j++) {
			activation += (h_activations[j])*(weights_ho[j][i]);
		}
		
		activation = activation*SIGMOID_SCALE*((double) MAX_HIDDEN/ (double) units);
		activations[i] = sigmoid(activation);
	}
}
...
void backprop_2layer(double sample[INPUTS],double h_activations[MAX_HIDDEN], double activations[OUTPUTS], double (*sigmoid)(double input), int label, double weights_ih[INPUTS][MAX_HIDDEN], double weights_ho[MAX_HIDDEN][OUTPUTS], int units)
{
   ...
   */
    ...
    * ************************************************************************************************/
	...
	int i, j, k;
	double target_correct, target_incorrect;
	double (*partial_derivative)(double input);
	double first_term, second_term, third_term;
	double target;
	double activation;
	double ais[OUTPUTS], h_ais[units];
	double weights_ho_cache[INPUTS][units];
	double second_terms[units][OUTPUTS], third_terms[units][OUTPUTS];
	...
	// Copy current weights into caches
	for (i = 0; i < OUTPUTS; i++) {
		for (j = 0; j < units; j++) {
			weights_ho_cache[j][i] = weights_ho[j][i];
		}
	}
	...
	for (i = 0; i < OUTPUTS; i++) {
		...
		ais[i] = activation*SIGMOID_SCALE*((double) MAX_HIDDEN/ (double) units);
	}
	for (i = 0; i < units; i++) {
		activation = 0;
		for (j = 0; j < INPUTS; j++) {
		...
		h_ais[i] = activation*SIGMOID_SCALE;
	}
	...
	for (i = 0; i < OUTPUTS; i++) {		
		...
		for (j = 0; j < units; j++) {
			// First term - I_a
			first_term = h_activations[j];
			
			// Second term - Partial derivative of activation fcn w.r.t. input
			second_term = partial_derivative(ais[i]);
			second_terms[j][i] = second_term;
			
			// Third term - (T_b - O_b)
			third_term = target - activations[i];
			third_terms[j][i] = third_term;
			
			// Weight update
			weights_ho_cache[j][i] += ALPHA*(first_term*second_term*third_term);
		}
	}
	...
	for (i = 0; i < units; i++) {	
		for (j = 0; j < INPUTS; j++) {
			...
			// Second term - Partial derivative of activation fcn w.r.t. input
			second_term = partial_derivative(h_ais[i]);
			
			// Third term
			third_term = 0;
			for (k = 0; k < OUTPUTS; k++) { // Loop over all output neurons
				third_term += weights_ho[i][k]*second_terms[i][k]*third_terms[i][k];
			}
			
			// Weight update 
			weights_ih[j][i] += ALPHA*(first_term*second_term*third_term);
		}
	}
	
	// Update weights with cached weight values
	for (i = 0; i < OUTPUTS; i++) {
		for (j = 0; j < units; j++) {
			weights_ho[j][i] = weights_ho_cache[j][i];
		}
	}
}
...
double logistic_partial_derivative(double input) {
	return logistic(input)*(1 - logistic(input));
}

double hyperbolic_partial_derivative(double input) {
